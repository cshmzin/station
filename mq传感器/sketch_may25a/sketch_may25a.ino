...
#define SSID    "TP-LINK_109"
#define PWD     "iloveyou15421542"
...
double temp = 0; //临时变量，存储A0读取的数据 
DS1302 rtc(RST,DAT,CLK);
IOTPWebSocketsClient webSocket("9b41d9c280e84ff989adcbbf2774c9ce",44);
...
void webSocketEvent(WStype_t type,uint8_t * payload,size_t length)
{
  switch(type)
  {
    ...
    case WStype_TEXT:
    {
        ...
        {
            char MQinfo[]="{\\\"MQ\\\":1111.11}";
            getMQ();
            sprintf(MQinfo, "{\\\"MQ\\\": %.2lf}",temp);
            webSocket.sendTXT(MQinfo,getTimeInfo());
        }
        else
        {
